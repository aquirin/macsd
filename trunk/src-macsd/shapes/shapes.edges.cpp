...
tuplax3<string> shapes::cannonical_edge(const CANDIDATE & x) const {
	assert(_nodos.find(x.first) != _nodos.end());
	assert(_nodos.find(x.second) != _nodos.end());
	assert(_desc_eje.find(x.third) != _desc_eje.end());

	tuplax3<string> res;

	res.first = tipoNodo(x.first);
	res.second = tipoNodo(x.second);
	res.third = _desc_eje[x.third];
...
cout << "INVALID " << _relacion_nodos.find(*p)->second << " " << q->first << " " << r->first << endl;
	  }
    }
    ...
    cout << "OP==" << endl << *this << endl << s << endl;
	  ...
	  if (igual(s)) {
	    ...
	    cout << "==I" << *this << endl << s << endl;
            ...
            posibilidades op(this, &s);

            for (posibilidades::iterator q = op.begin(); (q != op.end()) and !done; ++q) {
                cout << "!" << endl;
...
map<string, vector<CANDIDATE> > shapes::darPosibilidades() const {
    map<string, vector<CANDIDATE> > res, out_res;
    
    cout << "darPosibilidades:" << endl << *this << endl;

    // Paso la subestructura al map
    for (set< CANDIDATE >::iterator it =  _ejes.begin(); it != _ejes.end(); ++it) {
	//string nombre = _relacion_nodos.find((*it).first)->second + "_" + (*it).third + "_" + _relacion_nodos.find((*it).second)->second;
	unsigned int cual = _relacion_nodos.find((*it).first)->second;
	string txt1 = _desc_nodo[cual];
	cual = _relacion_nodos.find((*it).second)->second;
	string txt2 = _desc_nodo[cual];
	string txt3 = _desc_eje[(*it).third];
	string nombre = txt1 + "_" + txt2 + "_" + txt3;
	if (res.find(nombre) == res.end()) {
		//Nuevo
		res[nombre] = vector<CANDIDATE>();
	}
	res[nombre].push_back(*it);
    }
