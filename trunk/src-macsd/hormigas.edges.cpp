        ...
        while (p != _support.end()) {
            ...
            if (_instancias[*p].cantNodos() >= _subestructura.cantNodos()) {
                ...
                bool done = false;
                    ...
                    posibilidades op(&_subestructura, &_instancias[*p]);
                    for (posibilidades::iterator q = op.begin(); (q != op.end()) && !done; ++q) {
                        cout << "X" << endl;
	  ...
	  cout << "same2" << endl;
		...
		set<unsigned int> primero = _instancias[_support[pos]].nodosUtilizados();
                    ...
                    for (set<unsigned int>::const_iterator it = primero.begin(); it != primero.end(); ++it) {
                        ...
                        posibilidades op(&_subestructura, &_instancias[_support[pos]]);
                        posibilidades::iterator q;
    ...
    // 				cout << nn << ' ' << (*pp).first << ' ' << (*pp).second << endl;
                              ...
                              if (almenosuno) {
                                ...
                                for (unsigned int i = 0; (i < _support.size()) and !done; i++) {
                                  ...
                                  posibilidades opp(&posible_nueva_subestructura, &_instancias[_support[i]]);

                                  posibilidades::iterator qq = opp.begin();
                                  done = true;
