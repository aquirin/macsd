...
class MACS : public ACO {
    ...
    /**
	 * parametro de ejecucion beta para favorecer a una informacion heuristica
	 * respecto a otra
	 */
	float beta;
	
	/**
	 * parametro de ejecucion ro para la actualizacion de feromona
	 */
	float ro;
	
	/**
	 * parametro de ejecucion q0 para la regla de transicion del ACO
	 */
	float q0;
     ...
     */
      tuplax3<unsigned int> transicion(Hormiga &unaHormiga, unsigned int indice, vector< tuplax3<unsigned int> > &candidatas);
...
public: 
	 ...
	 */
	MACS(vector< pair<string,go> >& base, Parametros &params);
